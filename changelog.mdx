...
<Update label="June 11, 2026" description="Custom sending domains (Pro)" tags={["Dashboard"]}>
  ...
  address on a verified domain — without `@tx.senderkit.email` appearing to
  recipients and without any `amazonses.com` visible in DNS records.

  **How it works:**

  - Add a domain in **Dashboard → Channels → Add domain** (Pro-gated; free
    workspaces see an upgrade prompt).
  - Publish four DNS records: a DKIM TXT record at `senderkit._domainkey.{domain}`
    (white-label, no Amazon branding), an MX + SPF pair on `send.{domain}` for the
    custom MAIL FROM, and optionally a root SPF and DMARC record. The dashboard
    probes DNS live and shows a green **Set** indicator once each record is detected.
  - SenderKit verifies the domain via Amazon SES (polls every 5 minutes; **Check
    now** button for immediate re-check). Verification completes within minutes once
    records propagate; the SES window is 72 hours.
  - Once verified, the domain activates as a **managed-SES connection** in Channels.
