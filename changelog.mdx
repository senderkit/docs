...
<Update label="June 16, 2026" description="Starter plan, updated pricing" tags={["Dashboard", "Billing"]}>
  ...
  The built-in `@tx.senderkit.email` sender has per-plan monthly limits that protect
  the shared sending domain's reputation, independent of the plan's overall message
  quota. The Starter tier is now included:
  ...
  Connecting your own email provider (or a verified custom sending domain) removes the
  built-in sender cap entirely and routes through your connection instead.
