...
<Update label="June 26, 2026" description="Logs: rendered message preview in the drawer" tags={["Dashboard"]}>
  ## Rendered message preview in Logs

  The **Dashboard → Logs** detail drawer now shows you the rendered content of a
  sent message alongside the metadata — no need to re-send or dig through raw
  variables to see what a recipient actually received.

  - **Email** — the rendered HTML is displayed in a sandboxed iframe (no external
    resources loaded, no scripts). Subject line is shown above.
  - **SMS / push** — the final body text is shown in a preformatted block.
  - **Approximate badge** — when a message was sent using a template that contains
    a reusable block, the preview is marked **"approximate"** and a note is shown.
    Reusable block content expands live from the current version, so if a block was
    edited after the send, the preview may differ from what was delivered. Messages
    sent from a raw template or a pinned template version without reusable blocks
