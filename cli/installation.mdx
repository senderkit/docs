...
Report the active connection's live/test mode:
...
Useful in scripts when you want to confirm which key is in use before sending.
Add `--json` to get `{ mode, livemode }` as structured output.
