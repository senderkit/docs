...
<Note>
  Cc, Bcc, and attachments are supported by the API but can't be set from the
  CLI today — use the [TypeScript SDK](/sdks/typescript#send) (`cc`, `bcc`,
  `attachments` on `send()`) or the [HTTP API](/guides/http-api) for those.
</Note>
...
```bash
senderkit send-raw <to> --channel <email|sms|push> [content options]
```
...
<ParamField path="--channel <email|sms|push>" required>
...
<ParamField path="--from <string>">
  From override, e.g. `"Acme <hello@acme.com>"`. Defaults to your workspace's
  configured sender.
</ParamField>

<ParamField path="--reply-to <string>">
  Reply-To address. As with `senderkit send`, Cc/Bcc/attachments aren't
  available from the CLI — use the [SDK](/sdks/typescript#sendraw) or
  [HTTP API](/guides/http-api) for those.
</ParamField>
