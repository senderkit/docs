...
verified domain — no `@tx.senderkit.email` address visible to recipients, no
`amazonses.com` in DNS records.
...
| --- | --- | --- | --- |
| `senderkit._domainkey.{domain}` | TXT | DKIM public key (white-label — no `amazonses.com`) | Yes |
