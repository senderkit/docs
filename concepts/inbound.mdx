...
- Pass `localPart: "*"` for a **catch-all** address that receives mail for
  ...
  exact-match address always takes priority over the catch-all. A catch-all
  counts as one address against the plan cap like any other.
...
- `livemode` defaults to `true`. A test-mode address (`false`) still receives
  real mail, but only fans out to test-mode webhook endpoints and doesn't
  count against the message quota.
