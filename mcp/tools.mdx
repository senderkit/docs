...
<ParamField path="localPart" type="string">
  ...
  random one. Pass `"*"` for a catch-all that receives every local part on its
  domain that no exact address already claims. Max 64 characters.
</ParamField>
<ParamField path="description" type="string">
...
<ParamField path="webhookEndpointId" type="string">
  This workspace's webhook endpoint id to fire a `message.received` event to
  on receipt. Must be an active, live-mode endpoint owned by this workspace.
</ParamField>
...
<ParamField path="livemode" type="boolean">
  Defaults to `true`. A test-mode address still receives real mail, but fans
  out only to test-mode webhook endpoints and doesn't count against quota.
</ParamField>
