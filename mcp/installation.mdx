...
OAuth (for claude.ai and other OAuth-only clients), the hosted API-key endpoint,
and the local stdio server bundled in the CLI.

## Connect from claude.ai (OAuth)

If you use SenderKit from the **claude.ai** website or another OAuth-only MCP
client, you don't need an API key — the client handles auth through SenderKit's
OAuth 2.1 authorization server.

<Steps>
  <Step title="Open claude.ai → Customise → Add custom connector">
    Use whichever section your client uses to add MCP connections.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="The client reports an authentication failure.">
    For **OAuth** clients: the connection may have been revoked. Go to **Dashboard
    → Settings → Connected Apps**, revoke the entry for your client, and
    reconnect.
