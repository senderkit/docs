...
`metadata` you attached at send time. `search` does a case-insensitive substring
match over the message id, recipient, template, and `metadata` keys/values (max
512 characters).
...
// Or free-text search across id, recipient, template, and metadata:
// url.searchParams.set("search", "usr_123");
