...
The SenderKit dashboard supports two sign-in methods:

- **Email code (OTP)** — the default. Enter your email address and a
  one-time code is sent to your inbox.
- **Password** — an optional fallback. Set a password from **Account →
  Password** in the dashboard after signing in with a code. Once set, you
  can sign in with your email and password directly from the code screen
  ("Continue with password"), or reset a forgotten password via the
  "Forgot password?" link.

Password sign-up is intentionally not supported — a credential can only be
created from within an authenticated session, so a valid email address is
